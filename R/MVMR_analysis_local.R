...
MVMR_analysis_local <- function(ld_prune_file_dir=NULL,prefix=NULL,R,MVMR_method,
  ...
  files <- paste0(ld_prune_file_dir,prefix,".zmat.ldpruned.",seq(1,22),".RDS")
  X <- purrr::map_dfr(nb_files, readRDS)
  Z <- X %>% select(ends_with(".z"))
  ss <- X %>% select(ends_with(".ss"))
  se <- 1/sqrt(ss)
  p <- pnorm(-abs(as.matrix(Z))) * 2
  nms <- stringr::str_replace(names(Z), ".z", "")
  names(Z)<-names(ss)<-names(se)<-nms
  Rcor <- cov2cor(R)
  pmin <- apply(p[,-1, drop = F], 1, min)
  ix <- which(pmin < p_thresh)
  i <- ncol(Z)
  if(MVMR_method == "IVW"){
    hdat <-  list(exposure_beta = as.matrix(Z[ix, 2:i]),
                  exposure_pval = as.matrix(p[ix, 2:i]),
                  exposure_se = as.matrix(se[ix,2:i]),
                  outcome_beta = as.matrix(Z[ix,1]),
                  outcome_pval = p[ix,1],
                  outcome_se = se[ix,1],
    ...
    res <- run_grapple(beta.exposure =  Z[ix, 2:i],
                       beta.outcome = Z[ix,1],
                       se.exposure = se[ix,2:i],
                       se.outcome = se[ix,1],
                       R = Rcor)
  }else if(MVMR_method == "MRBEE"){
    res <- run_MRBEE(beta.exposure =  Z[ix, 2:i],
                     beta.outcome = Z[ix,1],
                     se.exposure = se[ix,2:i],
                     se.outcome = se[ix,1],
                     pleio_p_thresh = pleio_p_thresh, R = Rcor)
  }
}
