...
#' @param df_inst_counts Dataframe of the number of instruments for each trait. The required columns include `id` for trait ID, `n` for the number of instruments.
...
string_filter <- function(id.list,df_info,df_inst_counts,R2_cutoff = 0.9,
  ...
  df_select <- res_cluster %>% group_by(cluster) %>%
    slice_max(n, with_ties = FALSE) %>% ungroup()
