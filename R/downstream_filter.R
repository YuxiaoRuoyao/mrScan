...
#' @param method Bidirection MR or MVMR methods. Options: MR_IVW, MR_GRAPPLE,
...
#' @export
downstream_filter <- function(id_exposure,id.list,df_info,res,sig_level = 0.05,method){
  ...
  df_info[df_info$id %in% select_trait,"status"] <- paste0("select after downstream filtering by ",method)
  filter.trait <- id.list[!id.list %in% select_trait]
  df_info[df_info$id %in% filter.trait,"status"] <- paste0("delete in downstream filtering by ",method)
