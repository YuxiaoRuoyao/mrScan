...
R_ldsc <- function (Z_hat, ldscores, ld_size, N, return_gencov = FALSE,
                    make_well_conditioned = TRUE, cond_num = 1e+05 - 1, return_cor = TRUE)
{
  M <- ncol(Z_hat)
  J <- nrow(Z_hat)
  ...
  else if (class(N) == "numeric" | class(N) == "integer") {
    stopifnot(length(N) == M)
    N <- matrix(rep(N, each = J), nrow = J)
  }
  h2 <- lapply(1:M, function(m) {
    ix <- which(!is.na(Z_hat[, m]))
    snp_ldsc(ld_score = ldscores[ix], ld_size = ld_size,
             chi2 = Z_hat[ix, m]^2, sample_size = N[ix, m], blocks = NULL)
  })
  res <- expand.grid(trait1 = 1:M, trait2 = 1:M) %>% filter(trait1 <=
                                                              trait2)
  vals <- map2(res$trait1, res$trait2, function(i, j) {
    ...
    rg <- ldsc_rg(ld_score = ldscores[ix], ld_size = ld_size,
                  z1 = Z_hat[ix, i], z2 = Z_hat[ix, j], h2_1 = h2[[i]],
                  h2_2 = h2[[j]], sample_size_1 = N[ix, i], sample_size_2 = N[ix,
                                                                              j], blocks = NULL)
    c(rg[["int"]], rg[["gencov"]], rg[["gencorr"]])
  })
  val_int <- map(vals, 1) %>% unlist()
  res$value <- val_int
  res_copy <- filter(res, trait1 != trait2) %>% rename(n1c = trait2,
                                                       n2c = trait1) %>% rename(trait1 = n1c, trait2 = n2c)
  cov_mat <- bind_rows(res, res_copy) %>% reshape2::dcast(trait1 ~
                                                            trait2, value.var = "value")
  ...
  res_g_copy <- filter(res_gencov, trait1 != trait2) %>% rename(n1c = trait2,
                                                                n2c = trait1) %>% rename(trait1 = n1c, trait2 = n2c)
  gcov_mat <- bind_rows(res_gencov, res_g_copy) %>% reshape2::dcast(trait1 ~
                                                                      trait2, value.var = "value")
  Rg <- as.matrix(gcov_mat[, -1])
  res_gencor <- expand.grid(trait1 = 1:M, trait2 = 1:M) %>%
    filter(trait1 <= trait2)
  res_gencor$value <- map(vals, 3) %>% unlist()
  res_g_copy <- filter(res_gencor, trait1 != trait2) %>% rename(n1c = trait2,
                                                                n2c = trait1) %>% rename(trait1 = n1c, trait2 = n2c)
  gcor_mat <- bind_rows(res_gencor, res_g_copy) %>% reshape2::dcast(trait1 ~
                                                                      trait2, value.var = "value")
